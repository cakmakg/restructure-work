...
// --- ENDPOINT: PDF UPLOAD (S3 + Vektör) ---
app.post('/api/upload-pdf', upload.single('file'), async (req, res) => {
  try {
    if (!req.file) return res.status(400).send("Dosya yüklenmedi.");
    const { category } = req.body;

    // 1. ADIM: Önce dosyayı S3'e (LocalStack) gönderip sağlama alıyoruz!
    console.log("☁️ Dosya S3'e yükleniyor...");
    const s3Key = await uploadFileToS3(req.file);
    console.log(`✅ S3 Yükleme Başarılı. Key: ${s3Key}`);

    // 2. ADIM: PDF Parse işlemleri (Eski kodun aynısı)
    ...
    for (const chunk of chunks) {
      ...
      docsToSave.push({
        ...
        metadata: { 
          ...
          s3Key: s3Key, // <--- BURASI ÖNEMLİ: Artık verinin S3'teki yerini biliyoruz
          uploadedAt: new Date() 
        }
      });
    }
    ...
    res.json({ 
      message: `Dosya S3'e yedeklendi ve vektör veritabanına işlendi!`,
      s3Key: s3Key 
    });
...
// --- ENDPOINT: Mevcut Kategorileri Listele (Düzeltme) ---
app.get('/api/categories', async (req, res) => {
  try {
    // Veritabanındaki benzersiz (unique) kategori isimlerini bulur
    const categories = await Knowledge.distinct('metadata.category');
    
    // Boş veya null olanları temizle ve sırala
